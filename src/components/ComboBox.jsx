...
export default function ComboBox ({data, label, selectedElement, setSelectedElement}) {
		...
		<Combobox as="div" value={selectedElement} onChange={setSelectedElement}>
			...
			<div className="relative mt-2">
				<Combobox.Input
					...
					displayValue={(element) => element?.title}
				/>
				<Combobox.Button
								...
								{({active, selected}) => (
									<>
										<span className={classNames('block truncate',
										                            selected && 'font-semibold')}>{element.title}</span>
