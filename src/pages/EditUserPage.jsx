...
const EditUserPage = () => {
	...
	const [user, setUser] = useState({
		                                 name: oldUser?.name,
		                                 phone: oldUser?.phone,
		                                 pickup: oldUser?.pickUp,
		                                 dropoff: oldUser?.dropOff,
	                                 });
	...
	const postData = () => {
		try {
			...
			axiosInstance.put(`user/update/${oldUser._id}`, data)
			             .then((res => {
				             console.log(res);
				             if (res.status === 201) {
					             ...
					             setResponse({
						                         name: 'Success',
						                         message: `${user.name} updated successfully. (You will be automatically redirected to Users Page.)`
					                         });
					             clearState();
						...
						<form onSubmit={validateForm}>
							<div className="grid grid-flow-row auto-rows-max gap-8">
								...
								<div className="flex items-center gap-8">
									...
									{
										resetPassword ? (
											<p className="block text-sm leading-6 text-gray-900">Click submit to reset
												password.</p>
										) : (
											<SecondaryButton onClick={() => setResetPassword(true)}>Reset
												Password</SecondaryButton>
										)
									}
								</div>
								<div
